...
interface EditionPluginFormProps {
  title: string;
  pluginId?: string | null;
  onClose: () => void;
  showModal: boolean; // Ajout de showModal en tant que paramètre
}

interface EditionPluginFormValues {
  name: string;
  description: string;
  image: string;
  link: string;
...
const EditionPluginForm: React.FC<EditionPluginFormProps> = ({ pluginId, title, onClose, showModal }) => {
  const dispatch: AppDispatch = useDispatch();
  const plugins = useSelector((state: RootState) => selectPlugins(state));
  const [formValues, setFormValues] = useState<EditionPluginFormValues>({
    name: '',
    description: '',
    image: '',
    link: ''
...
export default EditionPluginForm;
